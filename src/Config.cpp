...
ConfigSection *Config::GetConfigSection(char *sectionName, char *sectionVal) {
  ...
  if (strcasecmp(sectionName, "basic") == 0) {
    ...
    newSec = basinSec;
  } else if (strcasecmp(sectionName, "lake") == 0) {
    TOLOWER(sectionVal);
    if (LakeConfigSection::IsDuplicate(sectionVal)) {
      ERROR_LOGF("Duplicate Lake section \"%s\"!", sectionVal);
      return newSec;
    }
    LakeConfigSection *lakeSec = new LakeConfigSection(sectionVal);
    g_lakeConfigs.insert(
        std::pair<std::string, LakeConfigSection *>(sectionVal, lakeSec));
    newSec = lakeSec;
    ...
    // Or maybe a lake calibration parameter set
    for (int i = 0; i < 1; i++) { // Only one lake type for now
      if (strcasecmp(sectionName, lakeCaliParamStrings[i]) == 0 || strcasecmp(sectionName, "lake_cali_param") == 0 || strcasecmp(sectionName, "lakecaliparams") == 0) {
        if (LakeCaliParamConfigSection::IsDuplicate(sectionVal)) {
          ERROR_LOGF("Duplicate Lake CaliParam section \"%s\"!", sectionVal);
          return newSec;
        }
        LakeCaliParamConfigSection *cpSec =
            new LakeCaliParamConfigSection(sectionVal);
        g_lakeCaliParamConfigs.insert(
            std::pair<std::string, LakeCaliParamConfigSection *>(
                sectionVal, cpSec));
        newSec = cpSec;
        break;
      }
