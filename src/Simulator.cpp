...
void Simulator::CleanUp() {
  ...
  // Close lake output files
  for (auto& kv : lakeOutputFiles) {
...
void Simulator::SimulateDistributed(bool trackPeaks) {
  ...
  // Lake inflow interception: for each lake, sum routed flow from all custom inlets if specified, otherwise use FAM-1 logic
  if (task->IsLakeModuleEnabled()) {
    for (auto* lake : lakes) {
      float inflowSum = 0.0f;
      if (task->GetBasinSec()->customLakeInlets && !lake->GetInlets().empty()) {
        // Use user-specified inlets
        for (const auto& inlet : lake->GetInlets()) {
          if (inlet.gridNodeIndex >= 0 && inlet.gridNodeIndex < (long)nodes.size()) {
            inflowSum += currentFF[inlet.gridNodeIndex];
          }
        }
      } else {
        // Default FAM-1 logic
        ...
        for (size_t i = 0; i < nodes.size(); ++i) {
          if (i == (size_t)lakeIdx) continue;
          long x = nodes[i].x;
          long y = nodes[i].y;
          if (g_FAM->data[y][x] == lakeFAM - 1) {
            if (nodes[i].downStreamNode == (unsigned long)lakeIdx) {
              inflowSum += currentFF[i];
            }
          }
        }
      }
      lake->SetInflow(inflowSum);
    }
  }

  // After lake inflow and outflow are set, enforce storage and hmax limits
  if (task->IsLakeModuleEnabled()) {
