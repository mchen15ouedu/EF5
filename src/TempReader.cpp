...
bool TempReader::Read(char *file, SUPPORTED_TEMP_TYPES type,
  ...
  if (tempGrid->cellSize < g_DEM->cellSize) {
    printf("ERROR: Temperature grid resolution (cellSize=%.2f) is higher than DEM resolution (cellSize=%.2f). Please resample your temperature data to match or be coarser than the DEM.\n", tempGrid->cellSize, g_DEM->cellSize);
    delete tempGrid;
    exit(1);
  }
