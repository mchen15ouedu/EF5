...
bool PETReader::Read(char *file, SUPPORTED_PET_TYPES type,
  ...
  if (petGrid->cellSize < g_DEM->cellSize) {
    printf("ERROR: PET grid resolution (cellSize=%.2f) is higher than DEM resolution (cellSize=%.2f). Please resample your PET data to match or be coarser than the DEM.\n", petGrid->cellSize, g_DEM->cellSize);
    delete petGrid;
    exit(1);
  }
